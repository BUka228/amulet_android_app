...
- Hilt управляет жизненным циклом Android‑классов (ViewModel/Activity), а внутри модулей Hilt вызывается `getKoin().get()` для получения зависимостей из Koin. Либо биндинг репозиториев делается Hilt‑модулем, который делегирует в Koin.
