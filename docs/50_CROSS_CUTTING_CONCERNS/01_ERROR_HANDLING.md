...
## 10. Архитектурные решения по размещению компонентов

### 10.1. Распределение по модулям

| Файл/Концепт                  | Модуль                                   | Причина                                                                 |
| ----------------------------- | ---------------------------------------- | ----------------------------------------------------------------------- |
| `AppError` и его наследники   | `:shared`                                | Доменная модель, нужна всем.                                            |
| `kotlin-result` зависимость   | `:shared`                                | KMP-совместимая библиотека.                                             |
| `ResultExtensions`, `Flow.successes` | `:shared`                                | Чистые KMP-утилиты для работы с `Result`.                               |
| `ExceptionMapper` (интерфейс) | `:shared`                                | Контракт для инверсии зависимостей.                                     |
| `Flow.asResult(mapper)`       | `:shared`                                | Использует KMP-контракт `ExceptionMapper`.                              |
| `safeApiCall`                 | `:core:network`                          | Работает с Retrofit/Android-зависимостями.                              |
| `mapHttpExceptionToAppError`  | `:core:network`                          | Работает с Retrofit/Android-зависимостями.                              |
| `BleExceptions`, `OtaExceptions` | `:core:ble`, `:core:ota` (если будет)  | Специфичные исключения инкапсулированы в своем модуле.                  |
| `ExceptionMapperImpl` (реализация) | `:app` (или `:data:common`)            | Собирает вместе все платформенные исключения для маппинга.              |
| Использование утилит          | `:data:*`, `:feature:*` (ViewModel)      | Потребление готовых, централизованных инструментов.                     |

### 10.2. Принципы размещения

1. **`:shared` модуль** - содержит только KMP-совместимые компоненты:
   - Доменные модели (`AppError`)
   - Чистые утилиты без платформенных зависимостей
   - Контракты для инверсии зависимостей

2. **`:core:*` модули** - содержат платформенные реализации:
   - Сетевые утилиты в `:core:network`
   - BLE-специфичные исключения в `:core:ble`
   - OTA-специфичные исключения в `:core:ota`

3. **`:app` или `:data:common`** - собирает все вместе:
   - Реализация `ExceptionMapperImpl`
   - DI-конфигурация
   - Платформенные зависимости

4. **`:data:*` и `:feature:*`** - потребляют готовые утилиты:
   - Используют `safeApiCall` для API-вызовов
   - Используют `Flow.asResult()` для Flow-операций
   - Используют extension-функции для `Result`
...
### 11.1. Использование JsonElement для парсинга ошибок
