...
18. **Начать план анимации (заголовок таймлайна):**
   ...
   ```
   BEGIN_PLAN:pattern_id:total_duration_ms[:PREVIEW]
   ```
   ...
   - опциональный флаг `PREVIEW` — если присутствует, клиент помечает загрузку как предпросмотр (preview) паттерна; прошивка может обрабатывать такие планы иначе, чем постоянные (см. реализацию устройства).
...
##### Проверка наличия практики (HAS_PLAN)

1. **Проверить, существует ли практика с заданным ID в энергонезависимой памяти устройства:**

   ```
   HAS_PLAN:pattern_id
   ```

   - `pattern_id` — строковый ID (до 64 символов), под которым практика должна быть сохранена в NVS;
   - при успешном наличии практики устройство отвечает `OK:HAS_PLAN`;
   - если практика с таким `pattern_id` отсутствует, устройство отвечает `ERROR:HAS_PLAN:NOT_FOUND[:reason]`;
   - любые другие ответы вида `ERROR:HAS_PLAN:...` на уровне клиента трактуются как "плана нет" и приводят к повторной загрузке плана через `BEGIN_PLAN/ADD_SEGMENTS/COMMIT_PLAN`.

##### Скрипты практик (PRACTICE_SCRIPT, v2.1 — экспериментально)

Описывает «логический сценарий практики», который ссылается на уже загруженные базовые паттерны (`pattern_id`) и задаёт порядок их воспроизведения.
Скрипт хранится отдельно от бинарных таймлайнов и имеет собственный идентификатор `practice_id`.

1. **Создать или обновить скрипт практики:**

   ```
   BEGIN_PRACTICE:practice_id
   ```

   - `practice_id` — строковый ID (до 64 символов), под которым сценарий практики будет сохранён в NVS;
   - если скрипт с таким `practice_id` уже существует, он должен быть перезаписан.

2. **Добавить шаг в скрипт практики:**

   ```
   ADD_PRACTICE_STEP:practice_id:order:pattern_id
   ```

   - `practice_id` — ID скрипта практики, ранее переданный в `BEGIN_PRACTICE`;
   - `order` — порядок шага (целое число, начиная с 1);
   - `pattern_id` — ID базового паттерна, ранее загруженного через `BEGIN_PLAN/ADD_SEGMENTS/COMMIT_PLAN`.

3. **Зафиксировать скрипт практики:**

   ```
   COMMIT_PRACTICE:practice_id
   ```

   - прошивка проверяет, что все упомянутые `pattern_id` существуют в хранилище планов;
   - в случае ошибок может вернуть `ERROR:COMMIT_PRACTICE:...`.

4. **Проверить наличие скрипта практики:**

   ```
   HAS_PRACTICE:practice_id
   ```

   - при успешном наличии скрипта устройство отвечает `OK:HAS_PRACTICE`;
   - если скрипт отсутствует, устройство отвечает `ERROR:HAS_PRACTICE:NOT_FOUND[:reason]`.

5. **Запустить скрипт практики по ID:**

   ```
   PLAY_PRACTICE:practice_id
   ```

   - устройство проигрывает шаги скрипта последовательно, для каждого шага поднимая соответствующий `pattern_id` из NVS;
   - при необходимости может отправлять уведомления вида `NOTIFY:PATTERN:STARTED:practice_id` и `NOTIFY:ANIMATION:COMPLETE:practice_id`.
