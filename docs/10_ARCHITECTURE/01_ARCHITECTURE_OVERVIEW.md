...
- Репозитории объединяют источники: Local first → Remote/Sync → Reconcile.
- Политика конфликтов:
  - Для критичных пользовательских данных (например, паттерны) — оптимистическая блокировка (поле `version`) и разрешение конфликта пользователем (см. раздел про редактор паттернов).
  - Для неконфликтных/перезаписываемых данных (например, имя профиля) — «server wins»/«last write wins».
