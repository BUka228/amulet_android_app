...
*   **`UserSessionUpdater` (интерфейс в `:shared`):**
    *   **Назначение:** Интерфейс для управления состоянием сессии. Реализуется в `:core:auth`, используется, как правило, внутри `AuthRepositoryImpl`.
    *   **Зависимости:** Располагается в `:shared` для единообразных контрактов между слоями.
    *   **Безопасность:** Несмотря на доступность из `:shared`, использовать в доменном слое (UseCase'ах) запрещено — только слой data.
...
#### UserSessionUpdater.kt (интерфейс в `:shared`)

```kotlin
/**
 * Интерфейс для управления состоянием сессии.
 * Определен в :shared как общий контракт между слоями.
 * Предназначен ИСКЛЮЧИТЕЛЬНО для использования в реализации репозитория аутентификации (слой data).
 */
